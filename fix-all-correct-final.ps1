$files = @("womens.html", "latest-releases.html", "user-profile.html", 
         "dist/womens.html", "dist/latest-releases.html", "dist/user-profile.html")
        ...
        (function() {
            ...
            if (accountLinks) {
                ...
                if (isLoggedIn) {
                    ...
                    accountLinks.innerHTML = `
                        <li style="display: block !important; visibility: visible !important; opacity: 1 !important; margin-bottom: 10px !important;">
                            <a href="login.html" style="display: block !important; color: #fff !important; text-decoration: none !important; padding: 8px 0 !important; visibility: visible !important; opacity: 0.7 !important;">LOGIN / REGISTER</a>
                        </li>
                    `;
                }
...
foreach ($file in $files) {
    if (Test-Path $file) {
        ...
        if ($file -eq "dist/user-profile.html" -or $file -eq "user-profile.html") {
            if ($content -match '<div class="cart-icon" onclick="window\.location\.href=''cart\.html''">\s+\?\?') {
                $content = $content -replace '<div class="cart-icon" onclick="window\.location\.href=''cart\.html''">\s+\?\?', "<div class=`"cart-icon`" onclick=`"window.location.href='cart.html'`">`n        🛒"
                $content | Set-Content -Path $file -Force
                Write-Host "Fixed cart icon in $file"
            }
        }
    } else {
